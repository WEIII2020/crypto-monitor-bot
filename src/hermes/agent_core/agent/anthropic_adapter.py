...
def _convert_to_bedrock_model_id(model: str) -> str:
    """Convert standard Anthropic model names to AWS Bedrock model IDs.

    Maps OpenRouter-style or standard Anthropic model names to AWS Bedrock format.
    Uses the most stable and widely available model versions.

    Examples:
        anthropic/claude-sonnet-4 -> us.anthropic.claude-sonnet-4-0-v1:0
        claude-3-5-sonnet -> anthropic.claude-3-5-sonnet-20240620-v1:0
        claude-3-haiku -> anthropic.claude-3-haiku-20240307-v1:0
    """
    # Normalize model name
    model_lower = model.lower().replace("anthropic/", "").replace(".", "-")

    # Bedrock model ID mapping - using stable, widely available versions
    # These are the base model IDs without regional prefixes (work in all regions)
    bedrock_model_map = {
        # Claude 4 models (if available in your region)
        "claude-sonnet-4": "us.anthropic.claude-sonnet-4-0-v1:0",
        "claude-opus-4": "us.anthropic.claude-opus-4-0-v1:0",

        # Claude 3.5 models - using stable v1 versions
        "claude-3-5-sonnet": "anthropic.claude-3-5-sonnet-20240620-v1:0",
        "claude-3-5-haiku": "anthropic.claude-3-haiku-20240307-v1:0",  # Most stable Haiku

        # Claude 3 models - original stable versions
    ...
    if model_lower in bedrock_model_map:
        return bedrock_model_map[model_lower]

    # Check partial match (for versioned names like claude-sonnet-4-0)
    for key, value in bedrock_model_map.items():
        if key in model_lower:
            return value

    # If already in Bedrock format, return as-is
    if "anthropic." in model or ":0" in model:
        return model

    # Default fallback to Claude 3 Haiku (most stable and widely available)
    logger.warning(f"Unknown model '{model}', defaulting to Claude 3 Haiku for Bedrock")
...
def build_anthropic_client(api_key: str, base_url: str = None):
    ...
    Returns an anthropic.Anthropic instance or anthropic.AnthropicBedrock if AWS Bedrock is detected.
    """
    ...
    # Check for AWS Bedrock environment variables
    # AWS Bedrock uses boto3 credentials, not HTTP endpoints
    if os.getenv("AWS_BEARER_TOKEN_BEDROCK") or (os.getenv("AWS_REGION") and not base_url and "bedrock" in api_key.lower()):
        logger.info("AWS Bedrock detected, using AnthropicBedrock client")
        try:
            # AnthropicBedrock reads AWS credentials from environment automatically
            return _anthropic_sdk.AnthropicBedrock(
                aws_region=os.getenv("AWS_REGION", "us-east-1"),
                timeout=Timeout(timeout=900.0, connect=10.0),
            )
        except Exception as e:
            logger.warning(f"Failed to create AnthropicBedrock client: {e}, falling back to standard client")
...
def convert_messages_to_anthropic(
    ...
    # AWS Bedrock does not support prompt caching — strip all cache_control
    # markers to avoid "unsupported model or request did not allow prompt caching" errors.
    if os.getenv("AWS_BEARER_TOKEN_BEDROCK") or os.getenv("AWS_REGION"):
        # Strip cache_control from system prompt
        if isinstance(system, list):
            for block in system:
                if isinstance(block, dict):
                    block.pop("cache_control", None)

        # Strip cache_control from all messages
        for m in result:
...
def build_anthropic_kwargs(
    ...
    # Convert to Bedrock model ID if using AWS Bedrock
    if os.getenv("AWS_BEARER_TOKEN_BEDROCK") or os.getenv("AWS_REGION"):
        model = _convert_to_bedrock_model_id(model)
        logger.info(f"Using Bedrock model ID: {model}")
